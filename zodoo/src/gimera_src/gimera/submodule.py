...
def _commit_submodule_inside_clean_but_not_linked_to_parent(
    main_repo, subrepo
):
    """
...
def _fetch_latest_commit_in_submodule(
    ...
    def _commit_submodule():
        ...
        if main_repo.path != repo.path:
            _commit_submodule_inside_clean_but_not_linked_to_parent(
                main_repo, repo
            )
    ...
    if sha:
        ...
        subrepo.out(
            "git", "reset", "--hard", f"origin/{repo_yml.branch}"
        ).strip()
...
def _temporary_switch_remote_to_cachedir(main_repo, repo_yml, relpath):
    with _get_cache_dir(main_repo, repo_yml) as cache_dir:
        main_repo.X(
            *(git + ["submodule", "set-url", relpath, f"file://{cache_dir}"])
        )
        try:
            yield
        finally:
            main_repo.X(
                *(git + ["submodule", "set-url", relpath, repo_yml.url])
            )


def _make_sure_subrepo_is_checked_out(
...
def __add_submodule(
    root_dir, working_dir, repo, config, all_config, common_vars
):
    ...
    if path.exists():
        ...
        try:
            ...
            if submodule.get_url(noerror=True) != config.url:
                repo.force_remove_submodule(
                    submodule.path.relative_to(repo.path)
                )
            else:
                return
    else:
    ...
    with _get_cache_dir(repo, config) as cache_dir:
        ...
        click.secho(
            f"Added submodule {relpath} pointing to {config.url}", fg="yellow"
        )
        if repo.staged_files:
            repo.X(
                *(
                    ...
                    + [
                        ...
                        "--no-verify",
                        "-m",
                        f"gimera added submodule: {relpath}",
                    ]
                )
            )
