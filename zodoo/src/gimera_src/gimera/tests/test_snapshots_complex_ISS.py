...
if basename == "test_snapshots_complex":
    for combo in combinations:
        ...
        code = code.replace(
            "__COMBO__ = [('I', 'S', 'S')]", f"__COMBO__ = [{combo}]"
        )
        code = code.replace(
            "test_snapshot_complex_ISS", f"test_snapshot_complex_{letters}"
        )
        path.write_text(code)
__COMBO__ = [("I", "S", "S")]


def test_snapshot_complex_ISS(temppath):
    ...
    for mode in ["use_gimera_migrate", "direct_snapshots"]:
        ...
        for i, adapted_paths in enumerate(
            ...
            for adapted_path in adapted_paths:
                ...
                # delete a file
                deleted_file = (
                    workspace_main / adapted_path / "dont_look_at_me"
                )
                deleted_file.unlink()
