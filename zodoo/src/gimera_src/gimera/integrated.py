    ...
    **options,
    ...
    with _get_cache_dir(main_repo, repo_yml, update=update) as cache_dir:
        ...
        with wait_git_lock(cache_dir):
            commit = (
                repo_yml.sha or repo_yml.branch
                if not update
                else repo_yml.branch
            )
        ...
        if any(
            ...
            try:
                ...
                click.secho(
                    ...
                    "It can be, that we will remove this message. ",
                    fg="yellow",
                )
                time.sleep(5)
        ...
        if repo_yml.edit_patchfile:
            _apply_patchfile(
                repo_yml.edit_patchfile_full_path,
                repo_yml.fullpath,
                error_ok=True,
            )
...
def _apply_merges(repo, repo_yml):
    ...
    for remote, ref in repo_yml.merges:
        ...
        click.secho(msg[-1])
        remote = [x for x in reversed(configured_remotes) if x.name == remote][
            0
        ]
