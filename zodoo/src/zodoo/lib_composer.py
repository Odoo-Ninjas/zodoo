...
@click.option(
    "--docker-compose",
    help="additional docker-compose files, separated by colon :",
)
        ...
        additional_config = (
            "\n".join(
                filter(
                    ...
                    map(
                        lambda x: x.strip(),
                        (additional_config or "").splitlines(),
                    ),
                )
            )
        ) + "\n"
...
def _copy_zodoo_src(ctx, config):
    src = config.dirs["images"] / "zodoo" / "src"
    dest = config.dirs["run.build.odoo"] / "zodoo_src"
...
def _download_images(config, images_url):
    ...
    click.secho("--------------------------------------------------")
    click.secho("Zodoo Status", fg="green", bold=True)
    click.secho("--------------------------------------------------")

    if not config.dirs["images"].exists():
        click.secho("Please install zodoo now with:", fg="red")
        click.secho(
    ...
    if config.restrict and config.restrict.get("docker-compose"):
        ...
        for d in [
            ...
            if not d.exists():
                if config.verbose:
                    click.secho(
                        f"Hint: you may use configuration file {d}",
                        fg="magenta",
                    )
                continue
            if d.is_file():
...
RUN chown -R odoo:odoo /opt/src && chmod -R a+r /opt/src
    """
