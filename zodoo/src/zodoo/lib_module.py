...
@click.option(
    "--uninstall",
    is_flag=True,
    help="Executes just uninstallation of modules.",
)
@click.option(
    "--test-tags",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Update all modules from MANIFEST install list (skip incremental since-git-sha detection)",
)
@click.option(
    "--stdout",
    is_flag=False,
...
def _uninstall_marked_modules(ctx, config, modules, no_restart=False):
    ...
    if modules:
        ...
        abort(
            f"Failed to uninstall - it is still installed: {','.join(modules)}"
        )
...
def _display_test_results(test_result, output_json):
    ...
    if output_json:
        ...
        if passed:
            click.secho(
                tabulate(passed, headers="keys", tablefmt="fancy_grid"),
                fg="green",
            )
        if errors:
            click.secho(
                tabulate(errors, headers="keys", tablefmt="fancy_grid"),
                fg="red",
            )
    if errors:
        sys.exit(-1)
    ...
    for module in module:
        ...
        data["modules"] = sorted(
            list(
                map(
                    lambda x: x.name,
                    modules.get_module_flat_dependency_tree(module),
                )
            )
            + [module.name]
        )
