...
"""
    ...
    def auth_connection(self):
        ...
        try:
            ...
            self.headers["SSL_CLIENT_S_DN_CN"] = client_cert_subject[
                "commonName"
            ]
    ...
    def new_websocket_client(self):
        ...
        if self.server.wrap_cmd:
            msg = "connecting to command: '{}' (port {})".format(
...
class WebSocketProxy(websockifyserver.WebSockifyServer):
    ...
    def started(self):
        ...
        if self.token_plugin:
            ...
            msg = "  - proxying from {} to {}".format(src_string, dst_string)
    ...
    def poll(self):
        ...
        if self.wrap_cmd and self.cmd:
            ...
            if ret != None:
                self.vmsg(
                    "Wrapped command exited (or daemon). Returned %s" % ret
                )
                self.cmd = None
...
def websockify_init():
    ...
    parser.add_option(
        "--verbose", "-v", action="store_true", help="verbose messages"
    )
    parser.add_option(
        "--traffic", action="store_true", help="per frame traffic"
    )
    parser.add_option(
        "--record",
    ...
    parser.add_option(
        "--cert", default="self.pem", help="SSL certificate file"
    )
    parser.add_option(
