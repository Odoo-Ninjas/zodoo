...
class TokenFileName(BasePlugin):
    ...
    def __init__(self, src):
        ...
        if not os.path.isdir(src):
            raise Exception(
                f"TokenFileName plugin requires a directory: {src}"
            )
...
class TokenRedis(BasePlugin):
    ...
    def __init__(self, src):
        ...
        try:
            ...
            logger.info(
                "TokenRedis backend initilized (%s:%s)"
        ...
        if stuff is None:
            ...
            if responseStr.startswith("{"):
                try:
                    ...
                    logger.error(
                        "Unable to decode JSON token: %s" % responseStr
                    )
                    return None
                except KeyError:
                    logger.error(
                        "Unable to find 'host' key in JSON token: %s"
                        % responseStr
                    )
                    return None
