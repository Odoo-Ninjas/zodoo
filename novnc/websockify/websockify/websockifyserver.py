...
class WebSockifyRequestHandler(
    WebSocketRequestHandlerMixIn, SimpleHTTPRequestHandler
):
    """
    ...
    * handler_id: A sequence number for this connection, appended to record filename
    ...
    def handle_websocket(self):
        ...
        self.log_message(
            "%s: %s WebSocket connection", client_addr, self.stype
        )
        if self.path != "/":
...
class WebSockifyServer:
    ...
    def __init__(
        ...
        if self.web:
            if self.file_only:
                self.msg(
                    "  - Web server (no directory listings). Web root: %s",
                    self.web,
                )
            else:
    ...
    def do_handshake(self, sock, address):
        ...
        if not handshake:
            ...
            if not os.path.exists(self.cert):
                raise self.EClose(
                    "SSL connection but '%s' not found" % self.cert
                )
            retsock = None
            try:
    ...
    def start_server(self):
        ...
        try:
                ...
                try:
                    try:
                        ...
                        if self.timeout and time_elapsed > self.timeout:
                            self.msg(
                                "listener exit due to --timeout %s"
                                % self.timeout
                            )
                            break
            ...
            # Close listen port
            self.vmsg(
                "Closing socket listening at %s:%s",
                self.listen_host,
                self.listen_port,
            )
            lsock.close()
