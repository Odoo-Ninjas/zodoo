...
class WebSocket:
    ...
    def connect(self, uri, origin=None, protocols=[]):
        ...
        if self._state == "response":
            ...
            if words[1] != "101":
                raise Exception(
                    "WebSocket request denied: %s" % " ".join(words[1:])
                )
    ...
    def _recvmsg(self):
        ...
        while self._recv_queue:
            ...
            if self.client and frame["masked"]:
                self.shutdown(
                    socket.SHUT_RDWR, 1002, "Procotol error: Frame masked"
                )
                continue
            ...
            if frame["opcode"] == 0x0:
                ...
                if self._partial_msg:
                    self.shutdown(
                        socket.SHUT_RDWR,
                        1002,
                        "Procotol error: Unexpected new frame",
                    )
                    continue
                ...
                if code is None:
                    ...
                    self.close_reason = (
                        "No close status code specified by peer"
                    )
                else:
