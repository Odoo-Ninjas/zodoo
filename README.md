...
| ODOO_FILES_COMMON=1                                      | Share the attachment filestore of all instances on the host via hardlinks into `filestore/_common` (saves space, keeps each database's own GC checklist). Migrate legacy symlinks with `odoo filestore unshare`. |
| ZODOO_DEVMODE=1                                          | `odoo setup upgrade` fast-forwards the currently checked-out branch in ~/.odoo/images instead of jumping to the latest release tag.                                                                     |
| ZODOO_ALPHA=1                                            | `odoo setup upgrade` tracks the `alpha` branch — opts in to the unstable feature channel (see "Alpha channel" below). Takes precedence over the latest-release-tag default; `ZODOO_DEVMODE` still wins. |

## Alpha channel (`ZODOO_ALPHA=1`)

The `alpha` branch in this repo is the staging area for unstable / experimental features that are not yet ready for `main`. To opt your installation in:
...
After setting it, `odoo setup upgrade` switches `~/.odoo/images` to the `alpha` branch and fast-forwards it on every upgrade (instead of jumping to the latest release tag). To leave the channel again, set `ZODOO_ALPHA=0` and run `odoo setup upgrade` once — that puts you back on the latest semver tag.

Precedence:

1. `ZODOO_DEVMODE=1` — pulls whatever branch you have checked out (wins over everything; for developers of zodoo itself)
2. `ZODOO_ALPHA=1` — `git checkout alpha` + fast-forward pull
3. neither set — production default: check out the highest `vX.Y.Z` tag
