...
This project uses **zodoo** (formerly wodoo) to manage the Odoo environment.
The CLI command is `odoo` (provided by the `wodoo` Python package, installed via pipx).

> **Important for AI assistants:** The `odoo` command here is the *zodoo CLI*, not the Odoo framework itself.
...
**Previous wodoo installation (cleanup):**
```bash
rm -Rf ~/.odoo/images
