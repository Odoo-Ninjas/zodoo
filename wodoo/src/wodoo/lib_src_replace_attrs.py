...
def odoo17attrs(root_dir):
            ...
            with open(xml_file, "rb") as f:
                ...
                for state_tag in tags_with_states:
                    ...
                    if (
                        ...
                        if not (
                            ...
                            or base_invisible.endswith("and")
                        ):
                            base_invisible = base_invisible + " or "
                        else:
                            base_invisible = base_invisible + " "
                    invisible_attr = "state not in [%s]" % ",".join(
                        ("'" + state.strip() + "'")
                        for state in state_tag["states"].split(",")
                    )
                ...
                for attribute_tag_states in attribute_tags_name_states:
                    ...
                    if existing_invisible_tag.text:
                        ...
                        if existing_invisible_tag.text.endswith(
                            ...
                            new_invisible_text = " or ".join(
                                [existing_invisible_tag.text, states_to_add]
                            )
                    else:
                ...
                with open(xml_file, "wb") as rf:
                    ...
                    for percent_d_result in percent_d_results.keys():
                        html = html.replace(
                            "'REPLACEME%s'" % percent_d_result,
                            percent_d_results[percent_d_result],
                        )
    ...
    if nofilesfound:
        ...
        click.secho("Succeeded on files", fg="green")
