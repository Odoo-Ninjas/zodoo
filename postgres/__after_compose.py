...
def suggest_postgres_conf(workload = "oltp", max_connections: int = 100, pg_version: int = 17):
    ...
    if platform.system() == "Linux":
        try:
            ...
            root_line = next((ln for ln in mounts.splitlines() if " / " in ln), "")
    ...
    work_mem = max(4 * 1024**2, int(total_ram * workmem_pool_frac) // int(consumers))
    ...
    print(f"# System: RAM={human_bytes(total_ram)}, cores={cores}, SSD={ssd}, workload={workload}")
