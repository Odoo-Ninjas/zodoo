...
const checkStreamBinding = (
  ...
  const allStreamInputsBound = Object.keys(inputs.streams[stream].pairs)
    .map((source) => inputs.streams[stream].pairs[source])
...
const checkSourceBinding = (
  ...
  const allSourceInputsBound = Object.keys(inputs.sources[source].pairs)
    .map((stream) => inputs.sources[source].pairs[stream])
    .every((inputName) => (state.inputs[inputName] || {})[screenId]);
...
export const BindingReducer = produce(
  (state: BindingState, inputs: InputState, action: BindingActionTypes) => {
    switch (action.type) {
