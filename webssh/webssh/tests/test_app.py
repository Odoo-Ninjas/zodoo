...
class TestAppBasic(TestAppBase):
    ...
    running = [True]
    sshserver_port = 2200
    body = "hostname=127.0.0.1&port={}&_xsrf=yummy&username=robey&password=foo".format(
        sshserver_port
    )  # noqa
    ...
    def test_app_with_user_keyonly_for_bad_authentication_type(self):
        ...
        self.assertEqual(response.code, 200)
        self.assert_status_in(
            "Bad authentication type", json.loads(to_str(response.body))
        )  # noqa

    @tornado.testing.gen_test
    def test_app_with_user_pass2fa_with_correct_passwords(self):
        self.body_dict.update(
            username="pass2fa", password="password", totp="passcode"
        )
        response = yield self.async_post("/", self.body_dict)
        self.assertEqual(response.code, 200)
        data = json.loads(to_str(response.body))
...
class TestAppWithRejectPolicy(OtherTestBase):
    ...
    def test_app_with_hostname_not_in_hostkeys(self):
        ...
        data = json.loads(to_str(response.body))
        message = "Connection to {}:{} is not allowed.".format(
            self.body["hostname"], self.sshserver_port
        )  # noqa
...
class TestAppNotFoundHandler(OtherTestBase):
    ...
    def test_with_not_found_put_request(self):
        response = self.fetch_request(
            "/pathnotfound", method="PUT", body=self.body
        )
        self.assertEqual(response.code, 404)
        self.assertEqual(
            response.headers["Server"], self.custom_headers["Server"]
