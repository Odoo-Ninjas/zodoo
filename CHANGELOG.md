...
- **Fix**: router vhost wizard: clarify the server_name, upstream_name and upstream_server prompts for first-time users
...
- **Fix**: Verliert ein Render die TLS-Einbindung eines vHosts, wird sie jetzt in Millisekunden zurueckgetragen statt in Stunden - und sieben bisher ungeschuetzte vHosts sind mit dabei.
  ...
  Seit der letzten Fassung holt zodoo eine verlorene Einbindung ueber "certbot install" zurueck. Das ist richtig, aber langsam: certbot laesst nur einen Lauf gleichzeitig zu, die Hosts werden also hintereinander abgearbeitet. Bei einer Aenderung an einem Template betrifft das die ganze Flotte - am 09.09.2026 gemessen: fuer die Domains zwischen "d" und "e" vergingen rund zehn Minuten, bei 149 Domains waeren das Stunden. Und solange der Lauf nicht durch ist, sind die noch nicht abgearbeiteten Domains ohne Verschluesselung erreichbar.

  Die verlorenen Zeilen stehen aber bereits in der Sicherungskopie, die vor dem Ausrollen angelegt wird, und sie waren bis Sekunden vorher in Betrieb. Sie zurueckzuschreiben ist eine reine Textaenderung: kein certbot, keine Nachfrage bei Let's Encrypt, kein Netz, keine Warteschlange. Uebernommen werden nur die fuenf Zeilen, die certbot selbst schreibt; einen von certbot angelegten Weiterleitungsblock laesst zodoo bewusst liegen, weil der nicht in einen anderen Server-Abschnitt hineingehoert. Die Pruefung mit "nginx -t" laeuft ohnehin unmittelbar danach, ein unbrauchbares Ergebnis wuerde also auffallen und zurueckgerollt. Sollte in der Sicherung keine verwertbare Zeile stehen, uebernimmt weiterhin certbot.

  Ausserdem greift der Schutz jetzt fuer alle vHosts, die eine Einbindung hatten - nicht nur fuer die mit gesetztem use_certbot. Auf hy-router tragen 154 vHosts eine Einbindung, aber nur 147 auch das Flag; die uebrigen sieben (darunter kraeuterblume.de mit seinen Umlaut-Schreibweisen) waeren beim naechsten Render ohne Verschluesselung zurueckgeblieben. vHosts mit ssl_self_signed bleiben ausdruecklich aussen vor: dort schreibt die Vorlage die TLS-Zeilen selbst, ein zusaetzlicher certbot-Block ergaebe ein zweites "listen 443" und nginx wuerde die Datei ablehnen.

  Zum Nachschauen: eine Vorlage unter router_global/templates aendern (etwa einen Kommentar) und "odoo router apply-vhosts --global" aufrufen. Der Lauf muss durchgehen, ohne dass certbot startet, und die Domains danach weiter ueber https antworten - "grep -c 'listen 443' sites-enabled/*" muss dieselbe Zahl wie vorher ergeben.


## 11.4.4

- Ein Render der vhost-Dateien nimmt einer Domain nicht mehr das HTTPS weg.

  Bei `use_certbot`-vhosts stehen `listen 443 ssl` und die Zertifikatspfade nicht in unseren Templates - die schreibt certbot in die ausgelieferte Datei (`# managed by Certbot`), und diese Datei ist die einzige Stelle, an der sie existieren. Solange der Render bitgleich zum letzten Stand ist, laesst sync_configs.py die Datei in Ruhe. Aendert sich am Template etwas, wird sie ueberschrieben - und der vhost hoert nur noch auf Port 80. Ohne Fehler, denn eine Konfiguration ohne TLS ist gueltig, und `nginx -t` laeuft sauber durch. Mit HSTS ist die Domain damit fuer Browser komplett unerreichbar.

  `odoo router vhost add` und `apply-vhosts` rendern immer die ganze Flotte, nicht nur den angefassten vhost. Am 09.09.2026 hat das nach einer Template-Korrektur 149 Domains gleichzeitig getroffen.

  Neu: nach jedem Render, der Dateien wirklich ueberschrieben hat, wird die Zertifikatseinbindung fuer genau die vhosts nachgezogen, die sie vorher hatten und jetzt nicht mehr (`certbot install`, kein ACME, keine Neuausstellung). Im Normalfall ist das keiner. Bleibt ein Host dabei ohne TLS, steht er als Klartext-Warnung im Protokoll statt unbemerkt offline.

  Ausserdem bricht `odoo router ssl` nicht mehr beim ersten fehlschlagenden Host ab. Bisher blieben alle dahinter liegenden vhosts unbearbeitet - wer das Kommando zum Wiederherstellen einer Flotte benutzte, stand mit einer halb versorgten Flotte da, ohne dass das Kommando das gemeldet haette. Jetzt laeuft er durch und zaehlt am Ende Erfolge und Fehlschlaege auf.

  Zum Testen: an einem Router mit certbot-vhosts eine Kleinigkeit am vhost-Template aendern und `odoo router apply-vhosts --global` laufen lassen. Danach muss `https://<domain>/` weiter antworten, und in `sites-enabled/<domain>` muessen die `# managed by Certbot`-Zeilen wieder stehen.
