...
def text(wert):
    """Mehrzeiliges zu einem Absatz zusammenziehen.

    Der Changelog ist eine Liste; ein Fragment darf trotzdem ausfuehrlich sein.
    Leerzeilen trennen Absaetze, innerhalb eines Absatzes werden die Zeilen
    ...
    for roh in str(wert).strip().split("\n\n"):
        zeilen = [z.strip() for z in roh.strip().split("\n") if z.strip()]
        if zeilen:
            absaetze.append(" ".join(zeilen))
    return "\n\n  ".join(absaetze)
